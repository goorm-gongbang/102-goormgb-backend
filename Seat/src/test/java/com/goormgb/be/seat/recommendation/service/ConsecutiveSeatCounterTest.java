...
class ConsecutiveSeatCounterTest {
	...
	private final ConsecutiveSeatCounter consecutiveSeatCounter = new ConsecutiveSeatCounter();
	...
	void 전체_연속_14칸에서_5연석() {
		...
		// when
		int count = consecutiveSeatCounter.countRealConsecutiveSeats(seats, 5);
	...
	void 중간_빠진_좌석_연속구간_분리() {
		...
		// then
		assertThat(count).isEqualTo(5); // [1~5]→1개, [7~14]→4개
	}
	...
	void 연석_불가_0반환() {
		...
		// when
		int count = consecutiveSeatCounter.countRealConsecutiveSeats(seats, 2);
	...
	void 좌석_없음_0반환() {
		// when
		int count = consecutiveSeatCounter.countRealConsecutiveSeats(List.of(), 3);
	...
	void 단일석_요청() {
		...
		// when
		int count = consecutiveSeatCounter.countRealConsecutiveSeats(seats, 1);
