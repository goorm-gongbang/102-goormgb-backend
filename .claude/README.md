...
| **stage** | QA/검증 브랜치. 배포 직전 검증용 |
| **dev** | 기능 통합 브랜치. feature들이 모이는 곳 |
...
3. **스테이징**: dev → stage PR
4. **배포**: stage → main PR
5. **운영 긴급 수정**: main → hotfixes/* 생성
6. **핫픽스 반영**: hotfixes/* → main 머지 후 **반드시 dev와 stage에도 동일 변경 반영**

> ⚠️ (6)이 빠지면 다음 배포 때 **핫픽스가 덮여서 버그 재발**
...
- 잔 커밋이 dev/stage/main 로그를 오염시키지 않음
...
| Allowed merge methods | Squash, Rebase | Squash, Rebase만 허용 |
